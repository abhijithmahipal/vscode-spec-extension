...
exports.TaskManager = exports.TaskStatus = exports.TaskComplexity = void 0;
...
class TaskManager {
    constructor(specPath) {
        ...
        this.tasks = new Map();
        this.taskStatusChangeEmitter = new vscode.EventEmitter();
        this.onTaskStatusChanged = this.taskStatusChangeEmitter.event;
    }
    /**
     ...
     */
    parseTaskMetadata(lines, startIndex) {
        const metadata = {
            requirements: [],
            dependencies: [],
            contextFiles: [],
    ...
    getTaskProgress() {
        ...
        const inProgressTasks = allTasks.filter((t) => !t.completed && t.progress && t.progress > 0).length;
        const blockedTasks = allTasks.filter((t) => !this.canExecuteTask(t.id)).length;
        return {
            totalTasks,
            completedTasks,
            inProgressTasks,
     ...
     * Requirement 5.2: Contextual prompts and clear next steps
     */
    generateContextualPrompts(task) {
        const prompts = [];
        // Basic execution prompt
        prompts.push(`Execute task: ${task.title}`);
        // Context-aware prompt with requirements
        if (task.requirements.length > 0) {
            prompts.push(`Execute task "${task.title}" addressing requirements: ${task.requirements.join(", ")}`);
        }
        // Prompt with context files
        if (task.contextFiles && task.contextFiles.length > 0) {
            prompts.push(`Execute task "${task.title}" considering context files: ${task.contextFiles.join(", ")}`);
        }
     ...
     */
    generateNextSteps(task, dependencyStatus) {
        const steps = [];
        if (!dependencyStatus.canExecute) {
            steps.push(`Complete dependencies first: ${dependencyStatus.blockedBy.join(", ")}`);
            return steps;
        }
     ...
     */
    async updateTaskStatus(taskId, newStatus, showConfirmation = true) {
        const task = this.tasks.get(taskId);
        if (!task) {
        ...
        if (showConfirmation && oldCompleted !== newCompleted) {
            ...
            const confirmed = await vscode.window.showInformationMessage(`${action === "complete" ? "✅" : "🔄"} ${action.charAt(0).toUpperCase() + action.slice(1)} task: "${task.title}"?`, { modal: true }, `Yes, ${action.charAt(0).toUpperCase() + action.slice(1)}`, "Cancel");
            if (confirmed !== `Yes, ${action.charAt(0).toUpperCase() + action.slice(1)}`) {
                return false;
            }
        }
        // Update task status
        ...
        if (newCompleted) {
            ...
            vscode.window.showInformationMessage(`✅ Task completed! ${progressMessage}\n${encouragement}`, progress.completedTasks === progress.totalTasks
                ? "🎉 All Done!"
                : "Continue");
        }
        else {
            vscode.window.showInformationMessage(`🔄 Task reopened. ${progressMessage}`, "Continue");
        }
        return true;
    }
    /**
    ...
    getNextRecommendedTask() {
        const availableTasks = Array.from(this.tasks.values()).filter((task) => !task.completed && this.canExecuteTask(task.id));
        ...
        availableTasks.sort((a, b) => {
            ...
            const aEnables = Array.from(this.tasks.values()).filter((t) => t.dependencies.includes(a.id)).length;
            const bEnables = Array.from(this.tasks.values()).filter((t) => t.dependencies.includes(b.id)).length;
    ...
    getTasksByStatus() {
        const allTasks = Array.from(this.tasks.values());
        return {
            available: allTasks.filter((t) => !t.completed && this.canExecuteTask(t.id)),
            blocked: allTasks.filter((t) => !t.completed && !this.canExecuteTask(t.id)),
            completed: allTasks.filter((t) => t.completed),
            inProgress: allTasks.filter((t) => !t.completed && t.progress && t.progress > 0),
        };
    }
    /**
