...
exports.UserGuidanceProvider = void 0;
...
class UserGuidanceProvider {
     ...
     */
    static async getContextualGuidance(phase, context) {
     ...
     */
    static async generateGuidance(phase, context) {
        switch (phase) {
     ...
     */
    static getRequirementsGuidance(context) {
        const steps = [];
        const tips = [
        ...
        if (!context.hasFiles) {
            steps.push({
                ...
                description: "Start by copying the requirements prompt and working with Copilot",
                action: {
     ...
     */
    static getDesignGuidance(context) {
        const steps = [];
        const tips = [
        ...
        if (!context.hasFiles) {
            steps.push({
                ...
                description: "Copy the design prompt and create your technical architecture",
                action: {
                    command: "specMode.copyPrompts",
     ...
     */
    static getTasksGuidance(context) {
        const steps = [];
        const tips = [
        ...
        if (!context.hasFiles) {
            steps.push({
                ...
                description: "Break down your design into actionable implementation tasks",
                action: {
                    command: "specMode.copyPrompts",
     ...
     */
    static getExecutionGuidance(context) {
        const steps = [];
        const tips = [
        ...
        if (completedTasks === 0) {
            ...
            steps.push({
                ...
                description: `${completedTasks}/${totalTasks} tasks completed (${Math.round(progress)}%)`,
                priority: "high",
                category: "next-step",
            });
        }
     ...
     */
    static getDefaultGuidance(context) {
        return {
            phase: specWorkflowManager_1.SpecPhase.REQUIREMENTS,
            steps: [
                {
     ...
     */
    static async showGuidance(guidance) {
        ...
        if (warnings.length > 0) {
            ...
            const action = await vscode.window.showWarningMessage(warning.title, { detail: warning.description }, "Fix Now", "Show All Issues", "Dismiss");
            if (action === "Fix Now" && warning.action) {
                await vscode.commands.executeCommand(warning.action.command, ...(warning.action.arguments || []));
            }
        ...
        if (nextSteps.length > 0) {
            ...
            const action = await vscode.window.showInformationMessage(`Next: ${nextStep.title}`, { detail: nextStep.description }, "Do It", "Show Tips", "Dismiss");
            if (action === "Do It" && nextStep.action) {
                await vscode.commands.executeCommand(nextStep.action.command, ...(nextStep.action.arguments || []));
            }
     ...
     */
    static async showAllIssues(issues) {
...
`;
        try {
            ...
            vscode.window.showErrorMessage(`Failed to show issues: ${error instanceof Error ? error.message : "Unknown error"}`);
        }
    }
    /**
     * Show helpful tips
     */
    static async showTips(tips) {
     ...
     */
    static getPhaseTips(phase) {
        const allTips = {
            [specWorkflowManager_1.SpecPhase.REQUIREMENTS]: [
            ...
            [specWorkflowManager_1.SpecPhase.DESIGN]: [
            ...
            [specWorkflowManager_1.SpecPhase.TASKS]: [
            ...
            [specWorkflowManager_1.SpecPhase.EXECUTION]: [
     ...
     * Addresses requirement 2.4
     */
    static validateInput(input, context) {
     ...
     */
    static getProgressEncouragement(completedTasks, totalTasks) {
        const progress = totalTasks > 0 ? (completedTasks / totalTasks) * 100 : 0;
        if (progress === 0) {
...
exports.UserGuidanceProvider = UserGuidanceProvider;
UserGuidanceProvider.GUIDANCE_CACHE = new Map();
UserGuidanceProvider.TIP_DISPLAY_INTERVAL = 30000; // 30 seconds
UserGuidanceProvider.lastTipTime = 0;
//# sourceMappingURL=userGuidance.js.map