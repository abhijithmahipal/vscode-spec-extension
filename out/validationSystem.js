...
class ValidationSystem {
     ...
     */
    static async validatePhase(phase, context) {
        this.initialize();
        const phaseRules = this.rules.filter((rule) => rule.phase === phase || rule.phase === specWorkflowManager_1.SpecPhase.REQUIREMENTS // General rules apply to all phases
        );
        const results = [];
        let passed = 0;
        let warnings = 0;
        ...
        const overallStatus = errors > 0 ? "failed" : warnings > 0 ? "warning" : "passed";
        return {
            phase,
            overallStatus,
            results,
     ...
     */
    static async showValidationResults(report) {
        const { summary, results } = report;
        if (summary.errors === 0 && summary.warnings === 0) {
            vscode.window.showInformationMessage(`✅ All ${summary.total} validation checks passed!`);
            return;
        }
        const errorResults = results.filter((r) => !r.result.passed && r.rule.severity === "error");
        const warningResults = results.filter((r) => !r.result.passed && r.rule.severity === "warning");
        // Show errors first
        if (errorResults.length > 0) {
            await this.showValidationIssues("Validation Errors", errorResults, "error");
        }
        // Then show warnings
        if (warningResults.length > 0) {
            await this.showValidationIssues("Validation Warnings", warningResults, "warning");
        }
    }
    /**
     * Show validation issues with fix actions
     */
    static async showValidationIssues(title, issues, severity) {
        ...
        const showMethod = severity === "error"
            ? vscode.window.showErrorMessage
            : vscode.window.showWarningMessage;
        const result = await showMethod(message, { detail: `${title}: ${firstIssue.rule.name}` }, ...actions);
        if (result === firstIssue.result.fixAction?.label &&
     ...
     */
    static async showDetailedValidationReport(title, issues) {
        const content = `# ${title}

${issues
            .map((issue, index) => `## ${index + 1}. ${issue.rule.name}
...
`;
        try {
            ...
            vscode.window.showErrorMessage(`Failed to show validation report: ${error instanceof Error ? error.message : "Unknown error"}`);
        }
    }
    /**
     * Requirements phase validation rules
     */
    static getRequirementsRules() {
        return [
            {
                id: "req-file-exists",
                    ...
                    return {
                        ...
                        fixAction: exists
                            ...
                            : {
                                ...
                                action: async () => {
                                    const template = this.getRequirementsTemplate(context.featureName);
                                    fs.writeFileSync(reqFile, template);
                                    await vscode.window.showTextDocument(vscode.Uri.file(reqFile));
                                },
                            },
                    };
                },
            },
            {
                ...
                check: async (context) => {
                    ...
                    const hasUserStories = content.includes("User Story:") || content.includes("As a");
                    return {
            ...
            },
            {
                ...
                description: "Requirements should include EARS format acceptance criteria",
                severity: "error",
                phase: specWorkflowManager_1.SpecPhase.REQUIREMENTS,
                check: async (context) => {
                    const content = context.files["requirements.md"] || "";
        ...
        return [
            {
                ...
                severity: "error",
                phase: specWorkflowManager_1.SpecPhase.DESIGN,
                check: async (context) => {
                    ...
                    return {
                        ...
                        fixAction: exists
                            ...
                            : {
                                ...
                                action: async () => {
                                    const template = this.getDesignTemplate(context.featureName);
                                    fs.writeFileSync(designFile, template);
                                    await vscode.window.showTextDocument(vscode.Uri.file(designFile));
                                },
                            },
                    };
                },
            ...
            },
            {
                ...
                check: async (context) => {
                    ...
                    const hasReqReferences = designContent.includes("requirement") ||
    ...
    static getTasksRules() {
        return [
            ...
            },
            {
                ...
                check: async (context) => {
                    const content = context.files["tasks.md"] || "";
                    const hasReqReferences = content.includes("_Requirements:") ||
                        ...
                        fixAction: specDirExists
                            ...
                            : {
                                ...
                                action: async () => {
                                    ...
                                    vscode.window.showInformationMessage(`Created spec directory: ${context.specPath}`);
                                },
                            },
                    };
                },
     ...
     */
    static getRequirementsTemplate(featureName) {
...
`;
    ...
    static getDesignTemplate(featureName) {
...
`;
    ...
    static getTasksTemplate(featureName) {
...
ValidationSystem.initialized = false;
