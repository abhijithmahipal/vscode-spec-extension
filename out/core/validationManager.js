...
class ValidationManager {
     ...
     */
    async validatePhase(phase, context) {
        const errors = [];
        const warnings = [];
        try {
            switch (phase) {
     ...
     */
    async validateRequirementsPhase(context) {
        const errors = [];
        const warnings = [];
        ...
        if (!requirementSections || requirementSections.length < 2) {
            warnings.push("Consider adding more requirements to cover different aspects of the feature.");
        }
     ...
     */
    async validateDesignPhase(context) {
        const errors = [];
        const warnings = [];
     ...
     */
    async validateTasksPhase(context) {
        const errors = [];
        const warnings = [];
        ...
        if (!taskMatches || taskMatches.length === 0) {
            ...
            warnings.push("Consider grouping related tasks or prioritizing the most important ones.");
        }
     ...
     */
    async validateExecutionPhase(context) {
        const errors = [];
        const warnings = [];
        const tasksFile = path.join(context.specPath, "tasks.md");
        if (!fs.existsSync(tasksFile)) {
        ...
        if (progress === 0) {
            warnings.push("No tasks have been completed yet. Start with the first task.");
        }
        else if (progress < 50) {
            warnings.push(`${Math.round(progress)}% complete. Keep up the good work!`);
        }
        else if (progress < 100) {
            warnings.push(`${Math.round(progress)}% complete. You're making great progress!`);
        }
        return { isValid: errors.length === 0, errors, warnings };
    }
    /**
    ...
    async validateGeneral(context) {
        ...
        if (context.featureName && !/^[a-z0-9-]+$/.test(context.featureName)) {
            warnings.push("Feature name should use kebab-case format (lowercase letters, numbers, and hyphens only).");
        }
        return { isValid: errors.length === 0, errors, warnings };
    }
    /**
     ...
     */
    validateUserStoryFormat(storyContent) {
        const errors = [];
        const warnings = [];
        // Check for proper user story format
        if (!storyContent.includes("As a") ||
            !storyContent.includes("I want") ||
            !storyContent.includes("so that")) {
            errors.push("User story should follow format: 'As a [role], I want [feature], so that [benefit]'");
        }
        // Check for acceptance criteria
     ...
     */
    getValidationSuggestions(phase) {
        switch (phase) {
            case workflowManager_1.SpecPhase.REQUIREMENTS:
                return [
