...
class ClipboardManager {
     ...
     */
    static async copyPromptWithEnhancements(prompt, title = "Prompt", context) {
        try {
            ...
            const action = await vscode.window.showInformationMessage(`Copy "${title}" to clipboard?`, {
                modal: false,
                detail: `Preview:\n${preview}\n\nLength: ${enhancedPrompt.length} characters`,
            }, "Copy to Clipboard", "Preview Full Content", "Save to File", "Cancel");
            switch (action) {
     ...
     */
    static async copyFromMultiplePrompts(prompts) {
        if (prompts.length === 0) {
            vscode.window.showWarningMessage("No prompts available to copy.");
            return { success: false, method: "cancelled" };
        }
        if (prompts.length === 1) {
            return await this.copyPromptWithEnhancements(prompts[0].content, prompts[0].title, prompts[0].context);
        }
        ...
        return await this.copyPromptWithEnhancements(selected.prompt.content, selected.prompt.title, selected.prompt.context);
    }
    /**
     * Enhance prompt with context and instructions
     * Addresses requirement 8.4
     */
    static enhancePromptWithContext(prompt, context) {
...
Generated by VSCode Spec-Driven Development Extension`;
     ...
     */
    static async handleLongPrompt(prompt, title) {
        const action = await vscode.window.showWarningMessage(`"${title}" is quite long (${prompt.length} characters). How would you like to proceed?`, {
            modal: true,
            detail: "Long prompts might be truncated in some contexts. Consider reviewing before copying.",
        }, "View & Edit", "Copy Anyway", "Save to File", "Cancel");
        switch (action) {
            case "View & Edit":
     ...
     */
    static async performClipboardCopy(prompt, title) {
        try {
            await vscode.env.clipboard.writeText(prompt);
            // Show success notification with next steps
            const action = await vscode.window.showInformationMessage(`✅ "${title}" copied to clipboard successfully!`, "Open Copilot Chat", "Dismiss");
     ...
     */
    static async showFullPreview(prompt, title) {
        try {
            ...
            const action = await vscode.window.showInformationMessage(`Preview of "${title}" opened. Copy to clipboard?`, "Copy to Clipboard", "Save to File", "Close");
            switch (action) {
                case "Copy to Clipboard":
                    return await this.performClipboardCopy(prompt, title);
                case "Save to File":
            ...
            vscode.window.showErrorMessage(`Failed to show preview: ${error instanceof Error ? error.message : "Unknown error"}`);
     ...
     */
    static async openPromptForEditing(prompt, title) {
        try {
            const doc = await vscode.workspace.openTextDocument({
                content: prompt,
                language: "markdown",
            ...
            const action = await vscode.window.showInformationMessage(`"${title}" opened for editing. Make your changes and then choose an action.`, "Copy Current Content", "Save to File", "Close");
            ...
            vscode.window.showErrorMessage(`Failed to open for editing: ${error instanceof Error ? error.message : "Unknown error"}`);
            return { success: false, method: "preview", error: String(error) };
        }
    }
    /**
     ...
     */
    static async savePromptToFile(prompt, title) {
        try {
            ...
            const action = await vscode.window.showInformationMessage(`✅ "${title}" saved to ${fileName}`, "Open File", "Copy File Content", "Dismiss");
            if (action === "Open File") {
            ...
            vscode.window.showErrorMessage(`Failed to save to file: ${error instanceof Error ? error.message : "Unknown error"}`);
     ...
     */
    static async handleClipboardFailure(prompt, title, error) {
        const errorMessage = error instanceof Error ? error.message : String(error);
        const action = await vscode.window.showErrorMessage(`Failed to copy "${title}" to clipboard: ${errorMessage}`, "Save to File", "Show in Preview", "Try Again", "Cancel");
        switch (action) {
            case "Save to File":
                return await this.savePromptToFile(prompt, title);
            ...
            case "Try Again":
                try {
                    await vscode.env.clipboard.writeText(prompt);
                    vscode.window.showInformationMessage(`✅ "${title}" copied successfully on retry!`);
                    return { success: true, method: "clipboard" };
                }
                catch (retryError) {
                    return {
     ...
     */
    static createPromptOption(id, title, content, description, context) {
        return {
            id,
            title,
            content,
...
ClipboardManager.MAX_PREVIEW_LENGTH = 200;
ClipboardManager.FALLBACK_FILE_NAME = "spec-prompt.md";
