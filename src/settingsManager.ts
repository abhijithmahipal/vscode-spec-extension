...
/**
 * Settings Manager for Spec-Driven Development Extension
 * Handles all configuration and user preferences
 * Requirements: 7.1, 7.5, 6.1
 */

export interface WorkflowBehaviorSettings {
  requirePhaseConfirmation: boolean;
  autoSaveOnPhaseTransition: boolean;
  showDetailedProgress: boolean;
  enableTaskDependencyValidation: boolean;
}

export interface UIPreferencesSettings {
  textWrapping: "auto" | "fixed" | "none";
...
export interface ExtensionSettings {
  workflowBehavior: WorkflowBehaviorSettings;
...
export class SettingsManager {
  ...
  private onSettingsChangedEmitter =
    new vscode.EventEmitter<ExtensionSettings>();
  ...
  private loadSettings(): ExtensionSettings {
    ...
    return {
      workflowBehavior: {
        ...
        enableTaskDependencyValidation: config.get(
          "workflowBehavior.enableTaskDependencyValidation",
          true
        ),
      },
      uiPreferences: {
      ...
      integration: {
        ...
        customTemplateDirectory: config.get(
          "integration.customTemplateDirectory",
          ""
        ),
      ...
      accessibility: {
        screenReaderSupport: config.get(
          "accessibility.screenReaderSupport",
          false
        ),
        keyboardNavigationEnhanced: config.get(
          "accessibility.keyboardNavigationEnhanced",
          false
        ),
      },
      notifications: {
   ...
   */
  public getWorkflowBehavior(): WorkflowBehaviorSettings {
   ...
   */
  public getUIPreferences(): UIPreferencesSettings {
   ...
   */
  public getIntegration(): IntegrationSettings {
   ...
   * Requirement 7.5: Accessibility support
   */
  public getAccessibility(): AccessibilitySettings {
  ...
  public async resetToDefaults(): Promise<void> {
    ...
    if (inspect) {
      ...
      const resetUser = await vscode.window.showQuickPick(
        ["Workspace only", "User settings too"],
        { placeHolder: "Reset which settings?" }
      );
  ...
  private async showWorkflowSettings(): Promise<void> {
    ...
    const options = [
      ...
      {
        label: `$(${
          current.enableTaskDependencyValidation ? "check" : "x"
  ...
  private async showUISettings(): Promise<void> {
    ...
    const options = [
      ...
      },
      {
        ...
        action: () =>
          this.updateSetting(
            "uiPreferences.showTooltips",
            !current.showTooltips
          ),
      },
      {
        label: `$(${
      ...
      },
      {
        ...
        action: () =>
          this.updateSetting("uiPreferences.compactMode", !current.compactMode),
      },
      {
        label: `$(${
          current.highContrastMode ? "check" : "x"
        }) High Contrast Mode`,
        description: current.highContrastMode ? "Enabled" : "Disabled",
        action: () =>
          this.updateSetting(
            "uiPreferences.highContrastMode",
  ...
  private async showIntegrationSettings(): Promise<void> {
    ...
    const options = [
      ...
      {
        label: `$(folder) Custom Template Directory: ${
          current.customTemplateDirectory || "Not set"
        }`,
        description: "Set custom template directory",
        action: async () => {
          const input = await vscode.window.showInputBox({
            prompt:
              "Enter custom template directory path (relative to workspace)",
   ...
   * Requirement 7.5: Accessibility support
   */
  private async showAccessibilitySettings(): Promise<void> {
    const current = this.getAccessibility();
    const options = [
      {
        label: `$(${
          current.screenReaderSupport ? "check" : "x"
      ...
      {
        label: `$(${
          current.keyboardNavigationEnhanced ? "check" : "x"
    ...
    if (selected) {
      ...
      if (
        selected.key === "accessibility.screenReaderSupport" &&
        selected.value
      ) {
        vscode.window.showInformationMessage(
          "Screen reader support enabled. The extension will now provide enhanced accessibility features."
        );
      }
    }
  }
  ...
  private async showNotificationSettings(): Promise<void> {
    ...
    const options = [
      ...
      {
        label: `$(${
          current.enableSoundNotifications ? "check" : "x"
        }) Sound Notifications`,
   ...
   * Requirement 6.1: VS Code design patterns
   */
  public applyThemeSettings(): void {
   ...
   */
  public getTextWrappingConfig(): { enabled: boolean; maxWidth?: number } {
  ...
  public shouldShowConfirmationDialog(
    type: "phaseTransition" | "taskCompletion" | "fileOperation"
  ): boolean {
    switch (type) {
