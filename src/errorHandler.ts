...
export enum ErrorCode {
  ...
  SPEC_NOT_ACTIVE = "SPEC_NOT_ACTIVE",
  ...
  TASK_DEPENDENCIES_NOT_MET = "TASK_DEPENDENCIES_NOT_MET",
...
export class SpecError extends Error {
  ...
  constructor(public readonly details: ErrorDetails, cause?: Error) {
...
export class ErrorHandler {
  private static readonly ERROR_HISTORY_LIMIT = 10;
  private static errorHistory: Array<{ error: SpecError; timestamp: Date }> =
    [];
   ...
   */
  static async handleError(error: Error | SpecError): Promise<boolean> {
   ...
   */
  private static convertToSpecError(error: Error): SpecError {
   ...
   */
  private static async showErrorWithRecovery(
    error: SpecError
  ): Promise<boolean> {
    const { details } = error;
    ...
    const actionLabels = [
      ...
      ...secondaryActions.slice(0, 2).map((a) => a.label), // Limit secondary actions
      "View Details",
      "Dismiss",
    ];
    ...
    const selectedAction = details.recoveryActions.find(
      (a) => a.label === result
    );
    if (selectedAction) {
      try {
        ...
        vscode.window.showInformationMessage(
          `✅ Recovery action completed: ${selectedAction.description}`
        );
        return true;
      } catch (recoveryError) {
   ...
   */
  private static createErrorDetail(error: SpecError): string {
    const { details } = error;
    const parts = [
   ...
   */
  private static async showErrorDetails(error: SpecError): Promise<void> {
   ...
   */
  private static createDetailedErrorReport(error: SpecError): string {
    const { details } = error;
...
${details.recoveryActions
  .map(
    (action) =>
      `### ${action.label}${action.primary ? " (Recommended)" : ""}
${action.description}`
  )
  .join("\n\n")}
...
`;
   ...
   * Log error for debugging
   */
  private static logError(error: SpecError): void {
    console.error(
      `[SpecError] ${error.details.code}: ${error.details.message}`,
      {
   ...
   */
  private static addToErrorHistory(error: SpecError): void {
  ...
  private static getFileNotFoundRecoveryActions(): RecoveryAction[] {
    return [
      {
  ...
  private static getPermissionDeniedRecoveryActions(): RecoveryAction[] {
    return [
      {
  ...
  private static getNoWorkspaceRecoveryActions(): RecoveryAction[] {
    return [
      {
        label: "Open Folder",
  ...
  private static getUnknownErrorRecoveryActions(): RecoveryAction[] {
    return [
      {
  ...
  static createWorkspaceError(message: string): SpecError {
    return new SpecError({
      code: ErrorCode.NO_WORKSPACE,
      message,
