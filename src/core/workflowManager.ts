...
export class WorkflowManager {
  ...
  async startSpec(featureName: string, originalIdea?: string): Promise<void> {
    try {
      ...
      if (fs.existsSync(specPath)) {
        const overwrite = await this.notificationManager.showConfirmation(
          `Spec "${featureName}" already exists. Do you want to overwrite it?`,
          ["Overwrite", "Choose Different Name", "Cancel"]
        );
        ...
        if (overwrite === "Choose Different Name") {
          const newName = await vscode.window.showInputBox({
            ...
            validateInput: (value) => {
              ...
              if (
                fs.existsSync(path.join(workspaceRoot, ".kiro", "specs", value))
              ) {
      ...
      await this.notificationManager.showSuccess(
        `🚀 Started spec for "${featureName}"! Requirements prompt is ready.`,
        ["Open Copilot Chat", "View Spec Panel"]
      );
    } catch (error) {
...
Save the output to .kiro/specs/${featureName}/requirements.md when complete.`;
  ...
  async markTaskComplete(taskId: string): Promise<boolean> {
    try {
      ...
      if (!taskFound) {
        throw new Error(
          `Task with ID "${taskId}" not found or already completed`
        );
      }
      ...
      const encouragement = this.getProgressEncouragement(
        completedTasks,
        tasks.length
      );
  ...
  private getProgressEncouragement(
...
Save to .kiro/specs/${this.currentFeature}/tasks.md when complete.`;
  ...
  public getTaskExecutionPrompt(task: TaskItem): string {
