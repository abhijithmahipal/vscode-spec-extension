...
`;
    ...
    for (const blockedTask of tasksByStatus.blocked) {
      console.log(
        `  🚫 ${
          blockedTask.title
        } - blocked by: ${blockedTask.dependencies.join(", ")}`
      );
    }
    ...
    if (recommended) {
      ...
      console.log(
        `  Dependencies: ${
          recommended.dependencies.length > 0
    ...
    if (availableTask) {
      ...
      if (originalShowInformationMessage) {
        require("vscode").window.showInformationMessage = async () =>
          "Yes, Complete";
      }
      ...
      try {
        ...
        console.log(
          "  ⚠️ Status update simulation (vscode mocking needed for full test)"
        );
      }
    }
    ...
    console.log(
      "  ✅ 5.5 - Intuitive controls with confirmation (structure ready)"
    );
