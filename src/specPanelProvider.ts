...
class TextFormatter {
  ...
  static wrapTextForTreeView(text: string, maxWidth: number = 50): string {
  ...
  static getResponsiveLengths(
    panelSize: "compact" | "normal" | "wide" = "normal"
  ) {
    const lengths = {
...
export class SpecPanelProvider implements vscode.TreeDataProvider<SpecItem> {
    ...
    if (!element) {
      ...
      if (!isActive) {
        return [
          new SpecItem(
            ...
            "Click to begin spec-driven development",
          ...
          new SpecItem(
            ...
            "Get context-aware prompts for each phase",
            vscode.TreeItemCollapsibleState.None,
            "info",
            undefined,
      ...
      items.push(
        new SpecItem(
          ...
          "feature-header",
          undefined,
          `Full feature name: ${currentFeature}`
        )
      );
      ...
      items.push(
        new SpecItem(
          ...
          `${phaseProgress} - ${phaseDescription}`,
          vscode.TreeItemCollapsibleState.None,
          "phase-indicator",
          undefined,
          `Current phase: ${phaseName}. Progress: ${phaseProgress}. ${phaseDescription}`
        )
      );
      ...
      if (currentPhase !== SpecPhase.EXECUTION) {
        const validationStatus = await this.getPhaseValidationStatus(
          currentPhase
        );
        ...
        if (
          ...
          items.push(
            this.createSectionHeader(
              "Issues to Resolve",
              `${validationStatus.issues.length} validation issues found`
            )
          );
          for (const issue of validationStatus.issues.slice(0, 3)) {
            ...
            items.push(
              new SpecItem(
                ...
                "validation-issue",
                undefined,
                `Validation Issue: ${issue}. This needs to be resolved before proceeding to the next phase.`
              )
            );
          }
          ...
          if (validationStatus.issues.length > 3) {
            items.push(
              new SpecItem(
                `⚠️ +${validationStatus.issues.length - 3} more issues`,
                "Additional validation issues need attention",
                vscode.TreeItemCollapsibleState.None,
                "validation-issue",
                undefined,
                `There are ${
                  validationStatus.issues.length - 3
                } additional validation issues that need to be resolved.`
              )
            );
          }
        }
      ...
      items.push(
        new SpecItem(
          "📋 Copy Prompts",
          "Choose from available prompts with preview",
          vscode.TreeItemCollapsibleState.None,
          "copy-prompts",
          {
      ...
      items.push(
        new SpecItem(
          "💡 Get Guidance",
          "Show contextual help and next steps",
          vscode.TreeItemCollapsibleState.None,
          "show-guidance",
          {
  ...
  private async getPhaseValidationStatus(phase: SpecPhase): Promise<{
    canProceed: boolean;
    issueCount: number;
    issues: string[];
  }> {
  ...
  private getPhaseHints(phase: SpecPhase): SpecItem[] {
    const hints: SpecItem[] = [];
    ...
    switch (phase) {
      ...
      case SpecPhase.DESIGN:
        hints.push(
          new SpecItem(
            "💡 Think architecture",
            "Consider components, data flow, and interfaces",
            vscode.TreeItemCollapsibleState.None,
            "hint",
            undefined,
          ...
          new SpecItem(
            ...
            "Ensure your design covers every requirement",
            vscode.TreeItemCollapsibleState.None,
            "hint",
            undefined,
      ...
      case SpecPhase.EXECUTION:
        hints.push(
          new SpecItem(
            "💡 One task at a time",
            "Focus on completing each task fully",
            vscode.TreeItemCollapsibleState.None,
            "hint",
            undefined,
  ...
  private createSeparator(): SpecItem {
    return new SpecItem(
      "─────────────────────",
      "",
      vscode.TreeItemCollapsibleState.None,
  ...
  private createSectionHeader(title: string, description: string): SpecItem {
    return new SpecItem(
      `▶ ${title}`,
      description,
      vscode.TreeItemCollapsibleState.None,
  ...
  private createProgressItem(
    ...
    return new SpecItem(
      ...
      "progress",
      undefined,
      `Progress: ${completed} out of ${total} items completed. ${percentage}% complete.`
    );
  }
...
export class SpecFilesProvider
  implements vscode.TreeDataProvider<SpecFileItem>
{
  private _onDidChangeTreeData: vscode.EventEmitter<
    SpecFileItem | undefined | null | void
  > = new vscode.EventEmitter<SpecFileItem | undefined | null | void>();
  readonly onDidChangeTreeData: vscode.Event<
    SpecFileItem | undefined | null | void
  > = this._onDidChangeTreeData.event;

  constructor(private workflowManager: SpecWorkflowManager) {}
  ...
  async getChildren(): Promise<SpecFileItem[]> {
    if (!this.workflowManager.isActive()) {
      return [];
    }
    ...
    return files.map(
      (file) =>
        new SpecFileItem(
          file.name,
          file.exists ? "✅ Created" : "⏳ Pending",
          file.path,
          file.exists ? "spec-file" : "spec-file-pending"
        )
    );
  }
}

export class SpecHelpProvider implements vscode.TreeDataProvider<HelpItem> {
  ...
  getChildren(): HelpItem[] {
    return [
      new HelpItem(
        "🚀 Getting Started",
        'Click "Start New Spec" to begin your first feature specification'
      ),
      new HelpItem(
        "📝 Requirements Phase",
      ...
      new HelpItem(
        "📋 Tasks Phase",
        "Break down design into small, actionable coding tasks"
      ),
      new HelpItem(
        "⚡ Execution Phase",
      ...
      new HelpItem(
        "🤖 Copilot Integration",
        "Context-aware prompts are automatically copied to clipboard"
      ),
      new HelpItem(
        "📁 File Structure",
        "All spec files are organized in .kiro/specs/{feature-name}/ directory"
      ),
    ];
  }
}
...
class SpecItem extends vscode.TreeItem {
  constructor(
    ...
    this.tooltip = TextFormatter.createMultiLineTooltip(
      label,
      description,
      additionalInfo
    );
  ...
  private getIconForContextValue(
    contextValue: string
...
class SpecFileItem extends vscode.TreeItem {
  constructor(
    ...
    super(
      TextFormatter.formatLabel(label),
      vscode.TreeItemCollapsibleState.None
    );
